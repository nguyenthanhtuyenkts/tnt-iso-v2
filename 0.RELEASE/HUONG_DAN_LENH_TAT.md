...
| `2`  | Tắt layer (`LAYOFF`)                | Chọn đối tượng thuộc layer cần tắt.                                                              |
| `3`  | Bật toàn bộ layer (`LAYON`)         | Gõ lệnh để bật lại tất cả layer đang tắt.                                                        |
...
| Lệnh  | Chức năng                            | Cách dùng / Ghi chú                                                                                                              |
| ----- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `VBB` | Tạo khung plot trong block khung tên | Chọn các block khung tên; lệnh vẽ rectangle trên layer `....11_TNT_A_PLOT`, lùi vào trong bounding box khoảng `15 x scale` và bỏ qua nếu đã có khung plot. |
...
| `A3` | Căn đuôi Leader và Text theo cùng hoành độ | Chọn Leader cùng Text, sau đó chọn điểm làm mốc X cho đuôi Leader và vị trí Text. |
| `A4` | Chỉnh đoạn cuối Leader nằm ngang       | Chọn các Leader cần hiệu chỉnh; lệnh đưa đoạn nối với chữ về phương ngang.    |
| `QLEADER` | Vẽ Leader trên layer chuẩn | Gọi QLEADER nhưng tự chuyển tạm về layer `....21_TNT_N_LEADER`, vẽ xong trả lại layer hiện hành trước đó. |
| `LE` | Vẽ Leader trên layer chuẩn | Lệnh tắt tương đương `QLEADER`. |
...
| `D5`   | Đặt hệ số đo tuyến tính cho Dimension         | Nhập tỷ lệ, chọn các Dimension; lệnh xóa text override và đặt `LinearScaleFactor = 1 / tỷ lệ nhập`. |
| `DDF`  | Tách liên kết Dimension được chọn             | Chọn các Dimension cần disassociate; lệnh đặt `DIMASSOC = 1` và tách liên kết cho vùng chọn.        |
| `DDA`  | Tách liên kết toàn bộ Dimension               | Quét toàn bản vẽ, tách liên kết tất cả Dimension tìm thấy và đặt `DIMASSOC = 1`.                    |
...
| Lệnh   | Chức năng                                           | Cách dùng / Ghi chú                                                                                              |
| ------ | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `B0`   | Đưa hình học trong block về layer `0`               | Chọn block cần chuẩn hóa; lệnh xử lý cả block lồng, đưa màu/linetype/lineweight trong block về ByBlock. Nên lưu bản vẽ trước khi chạy. |
...
| `CA`   | Cập nhật cao độ Attribute hàng loạt                 | Chọn Attribute cao độ mẫu trong block, sau đó chọn các block cùng tag; giá trị mới tính theo chênh tọa độ Y so với block mẫu. |
| `MATB` | Sao chép TextStyle của Attribute theo Tag           | Chọn block nguồn rồi các block đích; chỉ Attribute có Tag trùng với block nguồn mới được đổi TextStyle.          |
...
| Lệnh                       | Chức năng                                                                    | Cách dùng / Ghi chú                                                                                                   |
| -------------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `TNT_LAYER_SYNC_ALL`       | Chuyển toàn bộ layer cũ của dự án trước đó sang layer TNT ISO hiện tại       | Chạy trên bản sao hoặc sau khi lưu file; lệnh chuyển đối tượng, block definition và cố gắng xóa layer cũ.             |
| `TNT_LAYER_SYNC_SELECTION` | Tương tự chức năng trên nhưng chỉ áp dụng cho những vùng đối tượng được chọn | Chọn các đối tượng cấp cao nhất cần chuyển; phù hợp khi không muốn xử lý toàn bản vẽ.                                 |
| `TNT_LAYER_LISTING`        | Xuất danh sách layer của bản vẽ ra file text                                 | Tạo file `*_LAYER_LIST.txt` ngay cạnh file CAD hiện tại; mỗi dòng chỉ gồm tên layer.                                  |
| `TNT_BACKUP`               | Thiết lập backup tự động khi lưu bản vẽ                                      | Nhập số bản backup tối đa muốn giữ, không quá `10`; sau mỗi lần save, lệnh tạo bản copy timestamp trong cùng thư mục DWG và xóa backup cũ vượt số lượng. |
| `TNT_MEP_BASE`             | Chuẩn hóa block/đối tượng MEP sang layer base                                | Chọn block hoặc đối tượng rời; lệnh tạo block base độc lập khi cần, đưa đối tượng về `....46_TNT_BASE_MEP` và ép màu ByLayer. |
...
| Xử lý hàng loạt block/layer    | Lưu bản vẽ hoặc tạo bản sao trước khi dùng `B0`, `MAB`, `TNT_LAYER` và các lệnh `TNT_LAYER_SYNC_*`. |
